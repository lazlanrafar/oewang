...
// Factory function to create the Redis client (standard TCP Redis only —
// e.g. Docker or a self-hosted instance behind REDIS_URL).
const createRedisClient = () => {
  if (!REDIS_URL) {
    if (process.env.NODE_ENV !== "test") {
      throw new Error("Missing Redis configuration. Please provide REDIS_URL.");
    }
