...
test.describe("Finance: Income Categories", () => {
  ...
  test.beforeEach(async ({ page, dictionary }) => {
    ...
    await expect(
      page.getByRole("button", {
        name: dictionary.settings.category.income.add_button,
      }),
    ).toBeVisible({
      timeout: 15000,
    });
  ...
  test("should render the income categories page", async ({ page, dictionary }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: dictionary.settings.category.income.title,
        level: 2,
      }),
    ).toBeVisible({
      timeout: 10000,
  ...
  test("should show validation error for empty category name", async ({ page, dictionary }) => {
    ...
    await page
      .getByRole("button", {
        name: dictionary.settings.category.income.form.submit,
      })
      .click();
...
test.describe("Finance: Expense Categories", () => {
  ...
  test.beforeEach(async ({ page, dictionary }) => {
    ...
    await expect(
      page.getByRole("button", {
        name: dictionary.settings.category.expense.add_button,
      }),
    ).toBeVisible({
      timeout: 15000,
    });
  ...
  test("should render the expense categories page", async ({ page, dictionary }) => {
    ...
    await expect(
      page.getByRole("heading", {
        name: dictionary.settings.category.expense.title,
        level: 2,
      }),
  ...
  test("should show validation error for empty category name", async ({ page, dictionary }) => {
    ...
    await page
      .getByRole("button", {
        name: dictionary.settings.category.expense.form.submit,
      })
      .click();
