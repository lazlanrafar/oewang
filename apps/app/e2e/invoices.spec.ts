...
test.describe('Invoices Page', () => {
  ...
  test('loads invoices page successfully', async ({ page, dictionary }) => {
    ...
    const hasContent =
      (await page.getByRole('button', { name: dictionary.invoices.add_button }).first().isVisible().catch(() => false)) ||
      (await page.getByText(dictionary.invoices.summary.total).isVisible().catch(() => false)) ||
      (await page.getByText(dictionary.invoices.empty.title).isVisible().catch(() => false));
  ...
  test('displays invoice list or empty state', async ({ page, dictionary }) => {
    ...
    const hasEmptyState =
      (await page.getByText(dictionary.invoices.empty.title).isVisible().catch(() => false)) ||
      (await page.getByText(dictionary.invoices.empty.action).isVisible().catch(() => false));
    const hasSummary = await page.getByText(dictionary.invoices.summary.total).isVisible().catch(() => false);
  ...
  test('shows invoice actions menu', async ({ page }) => {
    ...
    if (count > 1) {
      ...
      if (await actionsButton.isVisible()) {
        ...
        await expect(
          page.getByRole('menuitem', { name: /edit|delete|view/i }).first()
        ).toBeVisible();
      }
    }
  });
