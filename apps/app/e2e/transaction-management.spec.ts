...
test.describe('Transaction Management', () => {
  ...
  test('user can open create transaction form', async ({ page, dictionary }) => {
    ...
    const plusIconButton = page
      .locator('button[class*="size-icon"], button[class*="h-9 w-9"]')
      .last();

    // Try clicking a recognizable create button
    const clicked = await (async () => {
      if (await page.getByTestId('create-transaction-btn').isVisible().catch(() => false)) {
  ...
  test('user can create an expense transaction', async ({ page, dictionary }) => {
    ...
    await expect(
      page.getByText(dictionary.transactions.toasts.created)
...
test.describe('Transaction Filtering', () => {
  ...
  test('can filter by date range via DateRangePicker', async ({ page }) => {
    ...
    const datePickerButton = page.locator('[class*="DateRangePicker"], button').filter({ hasText: /date|range|from/i }).first();
...
test.describe('Transaction List View', () => {
  ...
  test('displays transaction list or empty state', async ({ page }) => {
    ...
    const hasEmptyState = await page.getByText(/no transactions/i).isVisible().catch(() => false);
