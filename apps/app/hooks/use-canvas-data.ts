...
export function useCanvasData<T = unknown>(type: ArtifactType) {
  ...
  const messages = Array.isArray(chatData) ? chatData : ((chatData as Record<string, unknown>)?.messages as unknown[]) || [];
  ...
  const data = useMemo<T | null>(() => {
    ...
    for (let i = messages.length - 1; i >= 0; i--) {
      ...
      const artifactPart = parts?.find(
        (p) => p.type === "artifact" && p.artifactType === type,
      );
