...
import { getWhatsAppWebPublicInfoAction } from "@workspace/modules/integrations/integrations.action";

export function ConnectWhatsAppWeb({ dictionary }: { dictionary: Dictionary }) {
  ...
  const { data: publicInfo } = useQuery({
    queryKey: ["whatsappWebPublicInfo"],
    queryFn: async () => {
      const result = await getWhatsAppWebPublicInfoAction();
      return result.success ? result.data : null;
    },
  });
  ...
  // Assuming whatsapp-web doesn't require Pro, unlike Twilio. If it does, toggle this.
  const isPro = true; 
  
  const whatsappNumber = publicInfo?.phoneNumber || "";
  ...
  useEffect(() => {
    ...
    window.addEventListener("openWhatsAppWebConnect", handleOpen);
    return () => window.removeEventListener("openWhatsAppWebConnect", handleOpen);
  }, []);
  ...
  if (!whatsappNumber) {
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="overflow-hidden border-none bg-background p-0 sm:max-w-[480px]">
          <div className="p-7">
            <DialogHeader>
              <DialogTitle className="text-xl tracking-tight">WhatsApp (Web)</DialogTitle>
            </DialogHeader>
            <div className="py-8 text-center text-muted-foreground text-sm">
              The WhatsApp Web bot is not currently active. Please contact your administrator.
            </div>
          </div>
        </DialogContent>
      </Dialog>
          ...
          <DialogHeader>
            <div className="flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#25D366]/10">
                <Icons.WhatsApp className="h-5 w-5 fill-[#25D366]" />
              </div>
              <div>
                ...
                <DialogDescription className="text-muted-foreground mt-1">
                  Connect Oewang with your personal WhatsApp account. No Twilio required.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          {!isPro ? (
            ...
            <>
              ...
              <div className="grid w-full grid-cols-2 gap-4">
                <Button asChild variant="default" className="bg-[#25D366] hover:bg-[#1fb558] text-white">
                  <a
                    href={whatsappUrl}
                    target="_blank"
              ...
              <p className="max-w-[280px] text-center text-muted-foreground/80 text-xs leading-relaxed">
                Open your phone's camera to scan the QR code above and send the pre-filled connection message to link your workspace.
              </p>
            </>
          )}
        </div>

        <div className="mt-4 border-border/50 border-t bg-secondary/30 p-4">
