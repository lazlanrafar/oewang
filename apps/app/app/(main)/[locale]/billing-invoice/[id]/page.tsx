...
export default async function BillingInvoiceStandalonePage({
  ...
  return (
    <div data-print-only-root className="min-h-screen bg-muted/20 px-4 py-10 print:bg-background">
      <div className="mx-auto max-w-3xl space-y-6">
        {/* Toolbar — hidden when printing */}
        <div className="flex items-center justify-between print:hidden">
          <p className="font-mono text-[11px] text-muted-foreground uppercase tracking-widest">{inv.invoiceNumber}</p>
        ...
        <div className="print-document border bg-background p-10 shadow-sm print:shadow-none">
          ...
          <div className="mb-10 flex items-start justify-between border-b pb-8">
            <div>
              ...
              <p className="mt-1 font-mono text-muted-foreground text-xs">{inv.invoiceNumber}</p>
            </div>
            <div className="text-right">
              <p className="font-semibold text-[10px] text-muted-foreground uppercase tracking-widest">Issued</p>
          ...
          <div className="mb-10 grid grid-cols-2 gap-8">
            <div>
              <p className="mb-2 font-semibold text-[10px] text-muted-foreground uppercase tracking-widest">From</p>
            ...
            <div>
              ...
              <p className="font-medium text-sm">{inv.workspaceName || "Workspace"}</p>
              {inv.billingEmail && <p className="text-muted-foreground text-xs">{inv.billingEmail}</p>}
            </div>
          </div>
          ...
          {(inv.periodStart || inv.periodEnd) && (
            <div className="mb-10 grid grid-cols-2 gap-8 border-y py-4">
              <div>
                <p className="mb-1 font-semibold text-[10px] text-muted-foreground uppercase tracking-widest">Period</p>
              ...
              <div>
                ...
                <p className="font-medium text-xs capitalize">{inv.billingInterval ?? "—"}</p>
              </div>
            </div>
          )}
            ...
            <tbody>
              {inv.lineItems.map((li) => (
                <tr key={`${li.description}-${li.amount}`} className="border-b border-muted/40">
                  ...
                  <td className="py-4 pl-2 text-right font-mono">{formatMoney(li.amount, inv.currency)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="ml-auto w-full max-w-[280px] space-y-2 text-xs">
            ...
            {inv.taxAmount > 0 && (
              <div className="flex justify-between">
                ...
                <span className="font-mono">{formatMoney(inv.taxAmount, inv.currency)}</span>
              </div>
            )}
          ...
          <p className="mt-10 border-t pt-6 text-[10px] text-muted-foreground leading-relaxed">
            Paid via Mayar
            {inv.mayarTransactionId ? ` (txn ${inv.mayarTransactionId})` : ""}. This invoice was generated automatically
