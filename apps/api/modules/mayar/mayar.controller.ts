...
async function enqueueMayarWebhook(
  body: unknown,
  token?: string,
): Promise<void> {
...
// Takes MayarService as a parameter (defaulting to the real one) so tests can
// inject a fake instead of mock.module()-ing "./mayar.service" — Bun's
...
// real module, regardless of file load order (see mayar.service.test.ts).
export function createMayarController(mayarService = MayarService) {
  return new Elysia({
    prefix: "/mayar",
...
export const mayarController = createMayarController();
