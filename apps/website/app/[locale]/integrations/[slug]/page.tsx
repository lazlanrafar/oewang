...
export default async function IntegrationDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const related = integrations
    .filter((entry: PublicIntegration) => entry.category === item.category && entry.slug !== item.slug)
    .slice(0, 3);
          ...
          <div className="max-w-[1100px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-6 items-start">
              <div>
                ...
                <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground mb-3">{item.category}</p>
                <h1 className="font-serif text-4xl sm:text-6xl tracking-tight mb-4">{item.name}</h1>
