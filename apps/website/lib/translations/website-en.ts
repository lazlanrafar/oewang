export const websiteEn = {
  nav: {
    overview: "Overview",
    capture: "Capture",
    clarity: "Clarity",
    ai: "AI",
    workspaces: "Workspaces",
    signIn: "Sign in",
    signOut: "Sign out",
  },
  hero: {
    badge: "Daily finance tracker",
    title: "Track every rupiah without the spreadsheet.",
    subtitle:
      "oewang helps you input daily transactions, understand spending, and keep personal money clear with AI-assisted categorization.",
    ctaStartFree: "Start free trial",
    ctaSeeHow: "See how it works",
    ctaGetStarted: "Get started free",
    ctaGoToDashboard: "Go to Dashboard",
    ctaViewDemo: "View demo",
    trialNote: "14-day free trial · No credit card required",
    noCreditCard: "No credit card required",
  },
  capture: {
    label: "Capture",
    title: "Every transaction in seconds.",
    subtitle: "Input expenses, income, and receipts the moment they happen — from any channel.",
    transactions: {
      title: "Transactions",
      description: "Every daily expense and income entry, organized and categorized with less manual work.",
    },
    receipt: {
      title: "Receipt Capture",
      description: "Forward receipts via Telegram or WhatsApp. Oewang reads and files them automatically.",
    },
    invoices: {
      title: "Invoices",
      description: "Professional invoices, payments tracked automatically. Get paid faster.",
    },
  },
  clarity: {
    label: "Clarity",
    title: "All your transactions, unified.",
    subtitle:
      "Every income and expense tracked, searched, and categorized so your daily money stays clear.",
    items: [
      "Auto-categorization with AI",
      "Search and filter instantly",
      "Multi-currency support",
      "Bulk edit transactions",
    ],
  },
  ai: {
    label: "AI",
    title: "Ask anything about your finances.",
    subtitle: "Get instant answers about your spending, revenue, and trends. No more digging through reports.",
    items: ["Natural language queries", "Real-time insights", "Weekly summaries", "Multi-agent AI system"],
  },
  workspaces: {
    label: "Workspaces",
    title: "One account, many worlds.",
    subtitle: "Switch between personal, family, and team workspaces without mixing records.",
    items: [
      "Role-based member access",
      "Isolated records per workspace",
      "Shared dashboards",
      "Multiple workspaces per account",
    ],
  },
  cta: {
    title: "Ready to track money daily?",
    subtitle: "Use oewang to input transactions, understand spending, and stay ahead of bills.",
    getStarted: "Get started free",
    viewPricing: "View pricing",
    trialNote: "14-day free trial · No credit card required",
  },
  footer: {
    tagline: "Money clarity for real life.",
    rights: "All rights reserved.",
    lightMode: "Light mode",
    darkMode: "Dark mode",
  },
  notFound: {
    title: "404",
    heading: "Page not found",
    description: "The page you're looking for doesn't exist or has been moved.",
    goHome: "Go home",
    goToApp: "Go to app",
  },
};

export type WebsiteDictionary = typeof websiteEn;
