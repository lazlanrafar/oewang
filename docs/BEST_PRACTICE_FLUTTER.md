...
Transactions, wallets, and debts support cached reads and queued create/update operations. Categories and contacts are read-only picker caches. Budget status is cached by workspace, month, and year; budget mutations still require connectivity.

- **Storage:** Drift/SQLite schema v2 adds a monotonic local `revision` to writable rows and a budget-snapshot table. The v1 → v2 migration preserves pending writes. Generated files remain generated by `dart run build_runner build --delete-conflicting-outputs`.
- **Repositories:** offline decorators implement the existing abstract interfaces and accept abstract remote repositories for tests. Network errors fall back to cached data; authorization/server errors remain visible. Pending edits override remote rows, including edits that move a transaction outside a list's filter. Only complete response windows reconcile removed clean rows; partial pages do not erase other pages.
- **Sync:** `OewangApp` mounts `syncTriggerProvider`. Startup, connectivity recovery, app resume, and a five-second foreground timer drain the local queue. This timer inspects local pending records; it does not poll workspace data from the server. Per-row retry delay is 5s, 10s, …, capped at 5 minutes. No OS background execution is provided.
- **Dependencies:** wallets sync before transactions. Transactions referencing a still-pending wallet wait until that wallet is confirmed. Transaction creates use batches of at most 50 with string `amount` values matching the API contract.
- **Acknowledgements:** only the submitted revision may be cleared. A newer edit remains queued as an update. A replay returning an older create payload is followed by an update. Failure envelopes and missing acknowledgements never clear pending work. The conflict policy remains last-write-wins.
- **Isolation:** queue reads use the active workspace; a workspace change stops subsequent requests. Sync requests carry expected-workspace metadata that the auth interceptor checks against the attached JWT before transmission. The API still verifies the JWT and membership authoritatively. Late 401 responses from an old token do not clear a newer session.
- **UI:** `SyncStatusBanner` in `MainShell` shows the pending count across all tabs and expandable per-record errors. Status labels come from `core/localization/sync_strings.dart`.
- **Logout:** `SessionController.clear()` clears local tables, including queued writes and budget snapshots. Unsynced writes are therefore lost on explicit logout; preventing logout with pending work is not implemented. Data has to have been loaded once before it is available offline.

Simulator coverage also lives in `apps/native/integration_test/offline_sync_test.dart` (real SQLite, secure storage, encrypted loopback HTTP; a fixture backend). Unit/widget coverage lives in `sync_service_test.dart`, `offline_repositories_test.dart`, `offline_database_test.dart`, `sync_auth_test.dart`, `sync_trigger_test.dart`, and `widget/sync_status_banner_test.dart`. See [OFFLINE_SYNC_VERIFICATION.md](./MOBILE/OFFLINE_SYNC_VERIFICATION.md) for the verification scope and device checklist.
...
| `currency` | Labelled row → **keypad drawer** with the **workspace currency switcher**, live `IDR 1.000` (always the ISO code). The keypad shows one tab per workspace currency (main `IDR` + sub-currencies, read from the global `subCurrenciesProvider`); tabs auto-hide when the workspace tracks only its main currency. `amount`/`onAmountChanged`, `currency`, `valueColor`, `trailing`. |
| `amount`   | Same keypad as `currency` but **without** the currency tabs — a plain amount entry for screens where the currency is fixed or picked elsewhere (budget limit, wallet balance). Same props as `currency`. |
...
Row-layout opt-ins shared by `currency`/`amount`/`date`/`select`: `labelPosition` (`left` = two-column row with the label on the left, default; `top` = label stacked above the field) — in **both** layouts the value/input area renders the `variant` border (so use `variant: none` for the plain WMoney row; the border turns foreground while that field's drawer is open), `labelWidth`, `height` (uniform row height), `showBorder` (own bottom border instead of a `Divider`), `drawerId`, `placeholder`. They report changes through callbacks and never mutate the ViewModel themselves.

Folder structure under `atoms/inputs/`: `input.dart` (the widget) just **dispatches** by context — each context is **self-contained** in `contexts/`, holding its render code, its `open…Drawer` opener, and its sheet(s): `input_context_currency.dart` (serves both `currency` and `amount`; keypad + `openAmountDrawer`; the `AmountKeypad` is a `ConsumerStatefulWidget` that reads workspace currencies from `subCurrenciesProvider` for its tabs — pass `workspaceTabs: true` to enable them), `input_context_date.dart` (calendar + `openDateDrawer`), `input_context_select.dart` (`EntitySelect<T>` + grid/list pickers + `openGridDrawer`/`openListDrawer`), `input_context_text.dart`, and the shared `input_context_row.dart`. The stateful contexts (text/currency) receive their mutable state from `_InputState`. Shared building blocks live in `bases/`: `input_base_drawer_host.dart` (`FormDrawerHost`/`Scope`/`Controller`), `input_base_field_row.dart` (`FormFieldRow`), `input_base_drawer_header.dart`, `input_base_drawer_metrics.dart`. Plus `input_variant.dart`, `input_context.dart`, `input_label_position.dart`, `input_style.dart` (contract + `InputStyleResolver`, typed contexts only), `input_styles/` (one `InputStyle` per typed context), `decorators/`, `validators/`, `extensions/input_decoration_extension.dart`. Add a context → add a self-contained `contexts/` file + a dispatch arm; promote anything reused by 2+ contexts into `bases/`.

`FormFieldRow` (`atoms/inputs/bases/input_base_field_row.dart`) is the base labelled row (fixed-width label + child) that `Input`'s drawer contexts build on. For a text field in a left-labelled row, pass an `Input(variant: underline)` as the child (the Input owns the underline + focus highlight) — this is the canonical form look. For a non-`Input` value (read-only text, a chip row), use `underline: true` to draw the same value-column underline. Opt-ins: `height`, `showBorder` (full-width divider), `underline` (value-column underline, preferred over `showBorder` for forms), `focusNode`, `onTap`.
