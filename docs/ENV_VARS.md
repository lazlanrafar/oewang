...
| `AI_SERVICE_API_KEY` | apps/api, apps/app, apps/ai, apps/worker | Shared secret gating every hop between the TS side and the Python sidecar; `apps/worker` uses it to call `apps/ai`'s `/internal/*` (incl. `/internal/chat/stream`), `/import/extract`, `/draft/*`, and `/tools/execute` endpoints |
